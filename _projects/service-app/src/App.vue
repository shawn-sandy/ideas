<template>
  <div class="container mx-auto max-w-md p-4">
    ...
    <data-list :filter="{ userId: userIdFilter }" endpoint="posts">
      <div slot-scope="{ data: posts, error, load, loading }" class="mt-8">
        <h2>Post list</h2>
        <span class="block text-grey-darker text-sm mt-4">User</span>
        <select v-model="userIdFilter" class="shadow border rounded mt-1">
        ...
        <div class="mt-4">
          ...
          <post-list v-else :posts="posts" @updated="load" />
        </div>
      </div>
    </data-list>

    <data-model :id="1" endpoint="posts">
      <div slot-scope="{ data: post, loading }" class="mt-8">
        <h2>Single post</h2>
        <div v-if="post" class="rounded shadow-md p-4 mt-4">
          <span v-if="loading">Loading...</span>
    ...
    <data-model endpoint="posts">
      <div slot-scope="{ data: post, loading, create }" class="mt-8">
    ...
    <data-model :id="1" endpoint="posts">
      <div slot-scope="{ data: post, loading, update }" class="mt-8">
